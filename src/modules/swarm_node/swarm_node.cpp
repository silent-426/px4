...
void Swarm_Node::start_swarm_node()
{
    ...
    // 计算组中心（所有无人机同步跟随7点轨迹）
    float goal_x = hex_center_x;
    float goal_y = hex_center_y;

    // 所有无人机共用同一套7点轨迹切换逻辑
    switch (POINT_STATE) {
    case point0:  // 第1点（悬停点）
        goal_x = hex_center_x + hex_vertices[0][0];
        goal_y = hex_center_y + hex_vertices[0][1];
        break;
    case point1:  // 第2点
        goal_x = hex_center_x + hex_vertices[1][0];
        goal_y = hex_center_y + hex_vertices[1][1];
        break;
    case point2:  // 第3点
        goal_x = hex_center_x + hex_vertices[2][0];
        goal_y = hex_center_y + hex_vertices[2][1];
        break;
    case point3:  // 第4点
        goal_x = hex_center_x + hex_vertices[3][0];
        goal_y = hex_center_y + hex_vertices[3][1];
        break;
    case point4:  // 第5点
        goal_x = hex_center_x + hex_vertices[4][0];
        goal_y = hex_center_y + hex_vertices[4][1];
        break;
    case point5:  // 第6点
        goal_x = hex_center_x + hex_vertices[5][0];
        goal_y = hex_center_y + hex_vertices[5][1];
        break;
    ...
    // 计算目标位置 - 所有无人机使用相同的目标点（去掉队形概念）
    const float goal_z = begin_z - 20.0f; // 统一高度

    // 调试信息
    PX4_INFO("id=%d 第%d点目标: (%.2f, %.2f, %.2f)",
             vehicle_id, POINT_STATE + 1,
...
void Swarm_Node::Run()
{
    ...
    case state::TAKEOFF:
        // 所有无人机起飞到自身悬停点（第1点）
        if(control_instance::getInstance()->Control_posxyz(begin_x, begin_y, begin_z - 20)) {
...
int Swarm_Node::print_usage(const char *reason)
{
    ...
    PRINT_MODULE_DESCRIPTION("Swarm control module (6-drone version)");
